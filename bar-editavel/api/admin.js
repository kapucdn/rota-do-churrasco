...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (acao === "buscar_config") {
      const resposta = await fetch(`${SUPABASE_URL}/rest/v1/configuracoes_bar?select=*&id=eq.1`, {
        headers: cabecalhos,
      });
      const config = await resposta.json();
      return res.status(200).json({ config: config[0] || {} });
    }

    if (acao === "atualizar_config") {
      const resposta = await fetch(`${SUPABASE_URL}/rest/v1/configuracoes_bar?id=eq.1`, {
        method: "PATCH",
        headers: cabecalhos,
